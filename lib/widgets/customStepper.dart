...
class _StepperState extends State<CustomStepper> with TickerProviderStateMixin {
  ...
  Widget _buildHeaderText(int index) {
    ...
    if(
      widget.steps[index].modify != null && (
        widget.steps[index].state == StepState.complete || widget.steps[index].state == StepState.error
      )
    ) children.add(widget.steps[index].modify);
    ...
    return Stack(
      children: <Widget>[
        AnimatedCrossFade(
          ...
          secondCurve: const Interval(0.4, 1.0, curve: Curves.fastOutSlowIn),
          sizeCurve: Curves.fastOutSlowIn,
          crossFadeState: _isCurrent(index) && widget.steps[index].state != StepState.complete ? CrossFadeState.showSecond : CrossFadeState.showFirst,
          duration: kThemeAnimationDuration,
        ),
      ],
    );
