...
class ListaProductDetails extends StatefulWidget {
  final dynamic data;
  final dynamic listID;
  ListaProductDetails({Key key, this.data, this.listID}) : super(key: key);

  @override
  _ListaProductDetailsState createState() => _ListaProductDetailsState();
}

class _ListaProductDetailsState extends State<ListaProductDetails> {
  ...
  searchProduct() async {
    productResponse = ArticulosServices.getProductHybrisSearch(widget.data['code']);
    searchUrlShare();
    product = await productResponse;
    setState(() {
      if (product == "") {
        product = widget.data;
      }
      if (product != null) {
        ...
        imgList.add(NetworkData.hybrisBaseUrl + widget.data['images'][0]['url']);
      }
    });
  }
  ...
  showList() async {
    var ss = await showDialog(
        ...
        builder: (BuildContext context) {
          return ProductToList(upc: widget.data['code']);
        });
    print(ss);
    if (ss != null) {
      if (ss != 'error') {
    ...
    return WidgetContainer(
      Scaffold(
        ...
        body: Scaffold(
                                ...
                                child: Column(
                                  ...
                                  children: <Widget>[
                                    ...
                                    Container(
                                      ...
                                      child: Text(
                                        product["name"] != null ? product["name"] : product["summary"] != null ? product["summary"] : "",
                                      ...
                                      child: Column(
                                        ...
                                        children: <Widget>[
                                          Container(
                                            ...
                                            child: Text(
                                              widget.data['strikeThroughPriceValue'] != null ? "Precio Original: \$ " + formatter.format(double.parse(widget.data['strikeThroughPriceValue']["value"] != null ? widget.data['strikeThroughPriceValue']["value"].toStringAsFixed(2) : widget.data['strikeThroughPriceValue'].toStringAsFixed(2))) + " " + product['price']['currencyIso'] : "",
                                              textAlign: TextAlign.left,
                                              style: TextStyle(
                                                fontFamily: "Archivo",
                                                fontSize: widget.data['strikeThroughPriceValue'] != null ? 12 : 0,
                                            ...
                                            child: custom.ExpansionTile(
                                              ...
                                              children: <Widget>[
                                                Container(
                                                  ...
                                                  child: Html(defaultTextStyle: TextStyle(fontFamily: "Rubik", fontSize: 14, fontWeight: FontWeight.normal, color: HexColor("#637381"), height: 1.3), data: product['description']),
                                                ),
                                              ],
                                            ),
                                          ),
          ...
          bottomNavigationBar: BuyProduct(
...
class BuyProduct extends StatefulWidget {
  ...
  final String listID;
  BuyProduct({Key key, this.product, this.listID}) : super(key: key);
...
class _BuyProductState extends State<BuyProduct> {
  ...
  setUp() {
    ...
    conversion = widget.product["unit"]["conversion"].toString();

    if (code.toUpperCase().contains("KGM")) {
      pesable = "1";
    } else {
      pesable = "0";
    }
        ...
        child: !processingBuy
            ? FlatButton(
                ...
                onPressed: () async {
                  ...
                  var productCode = widget.product['code'];
                  num cartQtyFinal = widget.product['conversionFactor'] == null ? cartQty : cartQty * widget.product['unit']['conversion'];
                  print(productCode);
                  print(cartQtyFinal);
                  ListasServices.addProduct(widget.listID, productCode, cartQtyFinal.toString()).then((response) {
                    if (response == true) {
                ...
                child: Text(
                  "Agregar",
                  style: TextStyle(color: Colors.white, fontFamily: "Rubik", fontWeight: FontWeight.normal, fontSize: 16),
                ),
              )
            : FlatButton(
